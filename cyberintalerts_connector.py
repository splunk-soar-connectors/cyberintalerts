# File: cyberintalerts_connector.py
#
...
from cyberintalerts_consts import (
    CYBERINTALERTS_CONNECTIVITY_ENDPOINT,
    CYBERINTALERTS_ERR_CONNECTIVITY_TEST,
    CYBERINTALERTS_SUCC_CONNECTIVITY_TEST,
)


class CyberintalertsConnector(BaseConnector):
    """
    Cyberintalerts connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = CYBERINTALERTS_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(CYBERINTALERTS_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(CYBERINTALERTS_SUCC_CONNECTIVITY_TEST)
